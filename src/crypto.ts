...
async function getOrCreateSalt(): Promise<Uint8Array> {
  return new Promise((resolve) => {
    chrome.storage.local.get([SALT_STORAGE_KEY], (data) => {
      const stored = (data as Record<string, string | undefined>)[SALT_STORAGE_KEY];
      if (stored) {
...
export async function decryptApiKey(encoded: string): Promise<string> {
  if (!encoded.startsWith(ENC_PREFIX)) return encoded; // legacy plaintext — pass through

  const rest    = encoded.slice(ENC_PREFIX.length);
  const colonAt = rest.indexOf(':');
  if (colonAt === -1) throw new Error('Malformed encrypted key blob');
...
export function isEncrypted(value: string | undefined): value is string {
