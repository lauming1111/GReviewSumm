...
function isLocalProvider(settings: ReviewSettings): boolean {
  return (LOCAL_PROVIDERS as readonly string[]).includes(settings.aiProvider ?? 'ollama');
}
...
function resolveCharBudget(settings: ReviewSettings, depthChars: number): number {
  ...
  if (!isLocalProvider(settings)) return depthChars;
...
interface CompatOptions {
  /** Sampling parameters — omitted entirely for hosted providers. */
  params?: ModelParams;
  /**
   * Whether to include top_k / repeat_penalty. These are NOT part of the
   * OpenAI schema: local servers (LM Studio, llama.cpp, vLLM, …) accept them,
   * hosted APIs reject the request. Never set this for a cloud provider.
   */
  sendExtraParams?: boolean;
}

function buildCompatBody(
  model: string,
  prompt: string,
  opts: CompatOptions,
  withExtras: boolean
  ...
  if (withExtras) {
    ...
    if (p.repeatPenalty !== undefined) body.repetition_penalty = p.repeatPenalty;
  }
  return body;
}
...
async function callOpenAICompatible(
  ...
  const send = (withExtras: boolean) => fetchWithTimeout(url, {
    method: 'POST',
    headers,
